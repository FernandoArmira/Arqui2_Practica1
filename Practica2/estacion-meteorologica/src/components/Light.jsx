...
/**
 * 
 * @param {https://pacific-inlet-96370.herokuapp.com/statusultimodato
{"Velocidadviento": "Normal", "Visibilidad": "despejado" , "Lluvia": "Sin lluvia" , "Calor": "Sin calor"}
podrian agregar la fecha algo asi 'Fri 27 November' } value 
 * @returns 
 */

export const LightComponent = ({value = [], status = []}) => {
    
    const cadena = "Viento: " + status.Velocidadviento + " | Visibilidad: " + status.Visibilidad + " | Lluvia: " + status.Lluvia + " | Calor: " + status.Calor; //`Estado del Clima( Velocidad del Viento: ${status.status.Velocidadviento}, Visibilidad: ${status.status.Visibilidad}, Lluvia: ${status.status.Lluvia}, Calor: ${status.status.Calor} )`;
    //console.log(status)
  const data = {
    current: {
      date: `${value.fecha} ${value.hora}`,
      description: cadena,
